...
def plot_graphs(graph_type, bank_raw_target_perc, bank_target_perc):
    if bank_raw_target_perc.empty or bank_target_perc.empty:
        st.warning("Dados insuficientes para plotar os gráficos após os filtros.")
        return
    
    # Preparando os dados para plotagem
    def prepare_data(df):
        df = df.reset_index()
        df.columns = ['response', 'proportion']  # Renomeando as colunas
        return df
    
    bank_raw_prepared = prepare_data(bank_raw_target_perc)
    bank_prepared = prepare_data(bank_target_perc)
    ...
    if graph_type == 'Barras':
        ...
        sns.barplot(x='response', 
                    y='proportion', 
                    data=bank_raw_prepared, 
                    ax=ax[0], 
                    palette=["blue", "orange"])
        
        ax[0].set_title('Dados brutos', fontweight="bold")
        ax[0].set_xlabel('Resposta')
        ax[0].set_ylabel('Percentagem')
        ...
        ax[1].set_ylabel('Percentagem')
        
        # Adicionando os valores nas barras
        for container in ax[1].containers:
            ax[1].bar_label(container, fmt='%.2f%%', padding=3)

    else:  # Gráficos de pizza
        # Gráfico para dados brutos
        bank_raw_prepared.plot(kind='pie', 
                             y='proportion',
                             labels=bank_raw_prepared['response'],
                             autopct='%.2f%%', 
                             ax=ax[0], 
                             colors=["blue", "orange"],
                             legend=False)
        ...
        # Gráfico para dados filtrados
        bank_prepared.plot(kind='pie', 
                         y='proportion',
                         labels=bank_prepared['response'],
                         autopct='%.2f%%', 
                         ax=ax[1], 
                         colors=["blue", "orange"],
                         legend=False)
...
def main():
    ...
    if (data_file_1 is not None):
        ...
        if 'y' not in bank_raw.columns or 'y' not in bank.columns:
            st.error("A coluna 'y' não foi encontrada nos dados.")
            return
        ...
        try:
            plot_graphs(graph_type, bank_raw_target_perc, bank_target_perc)
        except Exception as e:
            st.error(f"Erro ao plotar gráficos: {str(e)}")
